...
PORT_DEFAULT = 4545
PSET_LENGTH_DEFAULT = 2
CSV_PATH_DEFAULT = './danfoss_torque_wrench_index.csv'


class TorqueWrenchSkill:
    __description__ = {
        'IN_EVENT': ["eventin"],
        'OUT_EVENT': ["eventout"],
    ...
    def INPUT_eventin(self, ip, port, pset_length, csv_path):
        ...
        pset_dict = dict()
        pset_dict["Pset"] = self.pset_length

        # Comment by Jeremy, need to remove this comment when the environment is available
        # client = TorqueWrenchSkill.AtlasClient(self.ip, self.port)
        # client.start_comm()
        ...
        status = 0x00000000
        print("Call TorqueWrenchSkill INPUT_eventin successfully")
        self.OUTPUT_eventout(status)

    def OUTPUT_eventout(self, status):
        pass
...
if __name__ == "__main__":
    torque_wrench_skill = TorqueWrenchSkill("TorqueWrenchSkillInstance")
    torque_wrench_skill.INPUT_eventin(IP_DEFAULT, PORT_DEFAULT, PSET_LENGTH_DEFAULT,CSV_PATH_DEFAULT)
